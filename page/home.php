<?php
	...
	// $tags  =  array('id','id_post','tag','status');
		$query = "SELECT tag_id,tag, post_id, title, description, UNIX_TIMESTAMP(date) AS timestamp FROM tags, posts WHERE tag_id = post_id ORDER BY date DESC LIMIT 10";
		$rst = mysqli_query($cnt, $query);

		while($arr = mysqli_fetch_array($rst)) {
			// echo '<a href="/post/'.slugify($arr['title']).'">' . $arr['title'] . '</a><br />';
			echo 'Title: <a style="color:orange;" href="/post/'.slugify($arr['post_id']).'">' . $arr['title'] . '</a><br />';
